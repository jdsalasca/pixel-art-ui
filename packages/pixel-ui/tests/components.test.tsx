...
import { PixelAmbientOcclusionControls, PixelAnimationAudit, PixelAnimationSheet, PixelArtifactStrip, PixelAssetCard, PixelAssetGrid, PixelBackgroundRemoval, PixelButton, PixelCheckboxGroup, PixelCleanupControls, PixelColorRampControls, PixelColorTemperatureControls, PixelCommandBar, PixelCompare, PixelConfirmDialog, PixelContactSheet, PixelDitherControls, PixelDustControls, PixelEffectStackPreview, PixelEmptyState, PixelEnhancementBatch, PixelFireControls, PixelFogControls, PixelFrameStrip, PixelGlowControls, PixelGrainControls, PixelKpi, PixelLeafFallControls, PixelLightningControls, PixelLibraryAudit, PixelLibrarySummary, PixelLibraryVariantPack, PixelLogViewer, PixelManifestAudit, PixelNotice, PixelOperationSummary, PixelPaddingControl, PixelPaletteStrip, PixelPresetStrip, PixelProgress, PixelProgressSteps, PixelQualityGate, PixelQualityMatrix, PixelRadioGroup, PixelRimLightControls, PixelRippleControls, PixelSchemaHint, PixelSceneAnimation, PixelSceneBundle, PixelSceneComposition, PixelScenePlan, PixelSceneRecommendations, PixelSelect, PixelShadowControls, PixelSilhouetteControls, PixelSlider, PixelSmokeControls, PixelSnowControls, PixelSpecularHighlightControls, PixelSpriteAnchors, PixelSpriteGeometry, PixelSpriteHitboxes, PixelSpriteNormalization, PixelSpriteRuntimeBundle, PixelTabs, PixelTextarea, PixelTimeline, PixelToast, PixelWaveControls, PixelWaterSprayControls, PixelWindSwayControls, pixelClassNames } from "../src/components.js";

describe("pixel-ui primitives", () => {
  it("renders typed isolated-pixel cleanup controls", () => {
    const markup = renderToStaticMarkup(<PixelCleanupControls minNeighbors={1} iterations={2} onMinNeighborsChange={() => undefined} onIterationsChange={() => undefined} />);
  ...
  it("renders typed dust controls", () => {
    const markup = renderToStaticMarkup(<PixelDustControls frames={5} seed={97} density={0.72} drift={0.25} rise={0.5} color="#C79A68" onFramesChange={() => undefined} onSeedChange={() => undefined} onDensityChange={() => undefined} onDriftChange={() => undefined} onRiseChange={() => undefined} onColorChange={() => undefined} />);
    expect(markup).toContain('aria-label="Dust overlay controls"');
    expect(markup).toContain("DENSITY · 0.72");
    expect(markup).toContain("RISE · 0.5");
  });

  it("renders typed leaf-fall controls", () => {
    const markup = renderToStaticMarkup(<PixelLeafFallControls frames={5} seed={103} density={0.68} wind={-0.2} color="#D97732" onFramesChange={() => undefined} onSeedChange={() => undefined} onDensityChange={() => undefined} onWindChange={() => undefined} onColorChange={() => undefined} />);
    expect(markup).toContain('aria-label="Leaf fall controls"');
    expect(markup).toContain("DENSITY · 0.68");
    expect(markup).toContain("WIND · -0.2");
  });

  it("renders typed water-ripple controls", () => {
    const markup = renderToStaticMarkup(<PixelRippleControls frames={5} seed={109} density={0.72} amplitude={3} color="#BDEBFF" onFramesChange={() => undefined} onSeedChange={() => undefined} onDensityChange={() => undefined} onAmplitudeChange={() => undefined} onColorChange={() => undefined} />);
    expect(markup).toContain('aria-label="Water ripple controls"');
    expect(markup).toContain("DENSITY · 0.72");
    expect(markup).toContain("AMPLITUDE · 3");
  });

  it("renders typed directional rim-light controls", () => {
    const markup = renderToStaticMarkup(<PixelRimLightControls color="#FFD166" direction="north" strength={0.75} onColorChange={() => undefined} onDirectionChange={() => undefined} onStrengthChange={() => undefined} />);
  ...
  it("renders typed specular-highlight controls", () => {
    const markup = renderToStaticMarkup(<PixelSpecularHighlightControls color="#FFFFFF" direction="north" radius={2} strength={0.8} onColorChange={() => undefined} onDirectionChange={() => undefined} onRadiusChange={() => undefined} onStrengthChange={() => undefined} />);
    expect(markup).toContain('aria-label="Sprite specular highlight controls"');
    expect(markup).toContain("DIRECTION · NORTH");
    expect(markup).toContain("RADIUS · 2");
    expect(markup).toContain("STRENGTH · 0.8");
  });

  it("renders typed color-ramp controls with bounded palette thresholds", () => {
    const markup = renderToStaticMarkup(<PixelColorRampControls shadowColor="#101020" midColor="#6080A0" highlightColor="#FFFFFF" shadowThreshold={0.3} highlightThreshold={0.7} onShadowColorChange={() => undefined} onMidColorChange={() => undefined} onHighlightColorChange={() => undefined} onShadowThresholdChange={() => undefined} onHighlightThresholdChange={() => undefined} />);
    expect(markup).toContain('aria-label="Sprite color ramp controls"');
    expect(markup).toContain("SHADOW THRESHOLD · 0.3");
    expect(markup).toContain("HIGHLIGHT THRESHOLD · 0.7");
  });

  it("renders typed seeded grain controls", () => {
    const markup = renderToStaticMarkup(<PixelGrainControls seed={17} intensity={0.8} scale={2} onSeedChange={() => undefined} onIntensityChange={() => undefined} onScaleChange={() => undefined} />);
    expect(markup).toContain('aria-label="Sprite grain controls"');
    expect(markup).toContain("SEED · 17");
    expect(markup).toContain("SCALE · 2");
  });

  it("renders typed Bayer dither controls", () => {
    const markup = renderToStaticMarkup(<PixelDitherControls darkColor="#202030" lightColor="#F0E8C8" strength={1} scale={2} onDarkColorChange={() => undefined} onLightColorChange={() => undefined} onStrengthChange={() => undefined} onScaleChange={() => undefined} />);
    expect(markup).toContain('aria-label="Sprite dither controls"');
    expect(markup).toContain("DARK COLOR");
    expect(markup).toContain("SCALE · 2");
  });

  it("renders typed sprite shadow controls with bounded offsets", () => {
    const markup = renderToStaticMarkup(<PixelShadowControls color="#000000" offsetX={2} offsetY={3} opacity={0.45} onColorChange={() => undefined} onOffsetXChange={() => undefined} onOffsetYChange={() => undefined} onOpacityChange={() => undefined} />);
    expect(markup).toContain('aria-label="Sprite shadow controls"');
    expect(markup).toContain("OFFSET X · 2");
    expect(markup).toContain("OPACITY · 0.45");
  });

  it("renders typed color temperature controls", () => {
    const markup = renderToStaticMarkup(<PixelColorTemperatureControls temperature={-1} intensity={0.7} onTemperatureChange={() => undefined} onIntensityChange={() => undefined} />);
    expect(markup).toContain('aria-label="Sprite color temperature controls"');
    expect(markup).toContain("TEMPERATURE · -1");
    expect(markup).toContain("INTENSITY · 0.7");
  });

  it("renders typed silhouette controls with bounded opacity", () => {
    const markup = renderToStaticMarkup(<PixelSilhouetteControls color="#08111F" opacity={0.6} onColorChange={() => undefined} onOpacityChange={() => undefined} />);
    expect(markup).toContain('aria-label="Sprite silhouette controls"');
    expect(markup).toContain("SILHOUETTE COLOR");
    expect(markup).toContain("OPACITY · 0.6");
  });

  it("renders typed wind sway controls with bounded animation values", () => {
    const markup = renderToStaticMarkup(<PixelWindSwayControls frames={6} seed={19} amplitude={2} direction="right" onFramesChange={() => undefined} onSeedChange={() => undefined} onAmplitudeChange={() => undefined} onDirectionChange={() => undefined} />);
  ...
  it("renders typed fog controls with bounded atmospheric values", () => {
    const markup = renderToStaticMarkup(<PixelFogControls frames={6} seed={23} density={0.72} drift={-0.35} color="#DDEBFF" onFramesChange={() => undefined} onSeedChange={() => undefined} onDensityChange={() => undefined} onDriftChange={() => undefined} onColorChange={() => undefined} />);
  ...
  it("renders typed smoke controls with bounded atmospheric values", () => {
    const markup = renderToStaticMarkup(<PixelSmokeControls frames={6} seed={41} density={0.75} drift={0.2} rise={0.8} color="#8A91A8" onFramesChange={() => undefined} onSeedChange={() => undefined} onDensityChange={() => undefined} onDriftChange={() => undefined} onRiseChange={() => undefined} onColorChange={() => undefined} />);
    expect(markup).toContain('aria-label="Smoke overlay controls"');
    expect(markup).toContain("DENSITY · 0.75");
    expect(markup).toContain("RISE · 0.8");
  });

  it("renders typed fire controls with bounded flame values", () => {
    const markup = renderToStaticMarkup(<PixelFireControls frames={6} seed={53} intensity={0.82} flicker={0.7} color="#FFD65A" onFramesChange={() => undefined} onSeedChange={() => undefined} onIntensityChange={() => undefined} onFlickerChange={() => undefined} onColorChange={() => undefined} />);
    expect(markup).toContain('aria-label="Fire overlay controls"');
    expect(markup).toContain("INTENSITY · 0.82");
    expect(markup).toContain("FLICKER · 0.7");
  });

  it("renders typed lightning controls with bounded storm values", () => {
    const markup = renderToStaticMarkup(<PixelLightningControls frames={6} seed={67} intensity={0.72} flash={0.8} color="#D8F3FF" onFramesChange={() => undefined} onSeedChange={() => undefined} onIntensityChange={() => undefined} onFlashChange={() => undefined} onColorChange={() => undefined} />);
    expect(markup).toContain('aria-label="Lightning overlay controls"');
    expect(markup).toContain("INTENSITY · 0.72");
    expect(markup).toContain("FLASH · 0.8");
  });

  it("renders typed wave controls with bounded shoreline values", () => {
    const markup = renderToStaticMarkup(<PixelWaveControls frames={6} seed={79} density={0.78} amplitude={3} color="#E6FAFF" onFramesChange={() => undefined} onSeedChange={() => undefined} onDensityChange={() => undefined} onAmplitudeChange={() => undefined} onColorChange={() => undefined} />);
    expect(markup).toContain('aria-label="Wave overlay controls"');
    expect(markup).toContain("DENSITY · 0.78");
    expect(markup).toContain("AMPLITUDE · 3");
  });
  ...
  it("renders typed background removal controls without a component superclass", () => {
    const markup = renderToStaticMarkup(<PixelBackgroundRemoval color="#142850" tolerance={12} connectedOnly onColorChange={() => undefined} onToleranceChange={() => undefined} onConnectedOnlyChange={() => undefined} />);
    expect(markup).toContain('aria-label="Background removal controls"');
    expect(markup).toContain("TOLERANCE · 12");
    expect(markup).toContain("CONNECTED TO EDGES ONLY");
  });
  ...
  it("renders enhancement batch outcomes and source guarantees", () => {
    const markup = renderToStaticMarkup(<PixelEnhancementBatch items={[{ filename: "hero.png", outputFilename: "hero-out.png", ok: true, passesApplied: ["cleanup"] }, { filename: "broken.png", outputFilename: "broken-out.png", ok: false, error: "decode failed" }]} summary={{ total: 2, succeeded: 1, failed: 1 }} />);
    expect(markup).toContain('aria-label="Enhancement batch report"');
  ...
  it("renders static and animated scene bundle artifacts", () => {
    const markup = renderToStaticMarkup(<PixelSceneBundle staticSrc="scene.png" staticManifestUrl="scene.json" animationSrc="scene.gif" animationManifestUrl="scene-animation.json" width={64} height={48} frames={8} delayMs={90} />);
    expect(markup).toContain("SCENE BUNDLE");
    expect(markup).toContain("scene.png");
    expect(markup).toContain("scene.gif");
    expect(markup).toContain("4 ARTIFACTS");
  });
});
