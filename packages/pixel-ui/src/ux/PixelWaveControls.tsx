...
export interface PixelWaveControlsProps extends HTMLAttributes<HTMLDivElement> {
  frames: number; seed: number; density: number; amplitude: number; color: string; disabled?: boolean;
  onFramesChange(value: number): void; onSeedChange(value: number): void; onDensityChange(value: number): void; onAmplitudeChange(value: number): void; onColorChange(value: string): void;
}

export function PixelWaveControls({ frames, seed, density, amplitude, color, disabled = false, onFramesChange, onSeedChange, onDensityChange, onAmplitudeChange, onColorChange, className = "", ...props }: PixelWaveControlsProps) {
  const safeFrames = Math.max(1, Math.min(24, Number.isInteger(frames) ? frames : 8));
  const safeSeed = Number.isInteger(seed) ? seed : 1;
  const safeDensity = Math.max(0, Math.min(1, Number.isFinite(density) ? density : 0.65));
  const safeAmplitude = Math.max(0, Math.min(8, Number.isFinite(amplitude) ? amplitude : 2));
  return <div className={pixelClassNames("pixel-wave-controls", className)} aria-label="Wave overlay controls" {...props}>
    ...
    <label className="pixel-wave-controls__color"><span>WAVE COLOR</span><input type="color" value={color} disabled={disabled} onChange={(event) => onColorChange(event.target.value)} /></label>
  </div>;
}
