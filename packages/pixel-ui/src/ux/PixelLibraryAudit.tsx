...
export function PixelLibraryAudit({ totalItems, totalCategories, totalPresets, totalFolders, readmePaths, previewPaths, spritePaths, valid, violations = [], className = "", ...props }: PixelLibraryAuditProps) {
  return <section className={pixelClassNames("pixel-library-audit", valid ? "pixel-library-audit--valid" : "pixel-library-audit--invalid", className)} aria-label="Asset library audit" {...props}>
    ...
    <div className="pixel-library-audit__paths"><span>README {readmePaths}/{totalItems}</span><span>PREVIEW {previewPaths}/{totalItems}</span><span>SPRITE {spritePaths}/{totalItems}</span></div>
