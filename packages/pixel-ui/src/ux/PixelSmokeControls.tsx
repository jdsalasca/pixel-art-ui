...
export function PixelSmokeControls({ frames, seed, density, drift, rise, color, disabled = false, onFramesChange, onSeedChange, onDensityChange, onDriftChange, onRiseChange, onColorChange, className = "", ...props }: PixelSmokeControlsProps) {
  ...
  const safeRise = Math.max(0, Math.min(1, Number.isFinite(rise) ? rise : 0.65));
  return <div className={pixelClassNames("pixel-smoke-controls", className)} aria-label="Smoke overlay controls" {...props}>
    ...
    <label className="pixel-smoke-controls__color"><span>SMOKE COLOR</span><input type="color" value={color} disabled={disabled} onChange={(event) => onColorChange(event.target.value)} /></label>
  </div>;
}
